 ...
 protected:
  ...
  void TestGlobalSortedCollection(const std::string& collection,
    ...
    auto SortedSetFunc = [&](const std::string& collection,
      ...
      return engine->SortedSet(collection, key, value);
    };
...
TEST_F(EngineBasicTest, TestUniqueKey) {
  ...
  for (const std::string& collection_name :
    ...
    // Set
    ASSERT_EQ(engine->SortedSet(collection_name, elem_key, new_val), ret_s);
    // Get
...
TEST_F(EngineBasicTest, TestBasicSnapshot) {
  ...
  auto WriteThread = [&](uint32_t id) {
    ...
    while (cnt--) {
      ...
      ASSERT_EQ(engine->SortedSet(sorted_collection, key1, key1), Status::Ok);
      ASSERT_EQ(engine->SortedSet(sorted_collection, key2, key2), Status::Ok);
    }
    // Wait snapshot done
    ...
    while (cnt--) {
      ...
      ASSERT_EQ(engine->SortedSet(sorted_collection, key1, "updated " + key1),
                Status::Ok);
      ASSERT_EQ(engine->SortedDelete(sorted_collection, key2), Status::Ok);
      ASSERT_EQ(engine->SortedSet(sorted_collection, key3, key3), Status::Ok);
      ASSERT_EQ(engine->SortedSet(sorted_collection_after_snapshot, key1, key1), Ok);
    }
  };
  ...
  for (auto is_opt : opt_restore_skiplists) {
    ...
    for (uint32_t id = 0; id < num_threads; id++) {
      ...
      while (cnt--) {
        ...
        ASSERT_EQ(got_v1, key1);
        ASSERT_EQ(got_v2, key2);
        ASSERT_EQ(backup_engine->SortedGet(sorted_collection_after_snapshot, key1,
                                      &got_v1),
                  Status::NotFound);
        ASSERT_EQ(engine->SortedGet(sorted_collection, key1, &got_v1), Status::Ok);
        ASSERT_EQ(engine->SortedGet(sorted_collection, key2, &got_v2), Status::Ok);
        ASSERT_EQ(engine->SortedGet(sorted_collection, key3, &got_v3),
                  Status::NotFound);
        ASSERT_EQ(got_v1, key1);
        ASSERT_EQ(got_v2, key2);
        ASSERT_EQ(engine->SortedGet(sorted_collection_after_snapshot, key1, &got_v1),
                  Status::NotFound);
      }
    }
...
TEST_F(EngineBasicTest, TestSeek) {
  ...
  ASSERT_EQ(engine->SortedSet(collection, zero_filled_str, zero_filled_str),
            Status::Ok);
  ...
  ASSERT_EQ(engine->CreateSortedCollection(collection), Status::Ok);
  ASSERT_EQ(engine->SortedSet(collection, "foo", "bar"), Status::Ok);
  ...
  ASSERT_EQ(engine->SortedSet(collection, "foo2", "bar2"), Status::Ok);
  engine->ReleaseSortedIterator(iter);
    ...
    for (int index_with_hashtable : {0, 1}) {
      ...
      auto SetupEngine = [&](uint32_t id) {
        ...
        for (int i = 1; i <= count; i++) {
          ...
          ASSERT_EQ(engine->SortedSet(global_skiplist, key, overall_val),
                    Status::Ok);
          ASSERT_EQ(engine->SortedSet(t_skiplist, key, t_val), Status::Ok);
          ASSERT_EQ(engine->SortedGet(global_skiplist, key, &got_val), Status::Ok);
...
TEST_F(EngineBasicTest, TestMultiThreadSortedRestore) {
  ...
  auto SetupEngine = [&](size_t id) {
    ...
    for (size_t i = 1; i <= count; ++i) {
      ...
      ASSERT_EQ(engine->SortedSet(average_skiplist, key, average_val), Status::Ok);
      ...
      ASSERT_EQ(engine->SortedSet(r_skiplist, key, r_val), Status::Ok);
      ...
      if ((rand() % i) == 0) {
        ...
        ASSERT_EQ(engine->SortedGet(average_skiplist, key, &got_val),
                  Status::NotFound);
        ASSERT_EQ(engine->SortedGet(r_skiplist, key, &got_val), Status::NotFound);
      }
    }
  };
...
TEST_F(EngineBasicTest, TestSortedHotspot) {
  ...
  for (const std::string& key : keys) {
    ASSERT_EQ(engine->SortedSet(collection_name, key, val1), Status::Ok);
    engine->ReleaseAccessThread();
      ...
      for (size_t i = 0; i < count; i++) {
        if (id % 2 == 0) {
          ...
          if (id % 4 == 0) {
            ...
            ASSERT_EQ(engine->SortedSet(collection_name, key, val2), Status::Ok);
          }
        } else {
          // Odd Read
          std::string got_val;
          ASSERT_EQ(engine->SortedGet(collection_name, key, &got_val), Status::Ok);
          bool match = false;
          match = match || (got_val == val1);
          match = match || (got_val == val2);
          if (!match) {
...
TEST_F(EngineBasicTest, TestSortedCustomCompareFunction) {
  ...
  for (size_t i = 0; i < collections.size(); ++i) {
    auto Write = [&](size_t) {
      for (size_t j = 0; j < count; j++) {
        ASSERT_EQ(engine->SortedSet(collections[i], key_values[j].first,
                               key_values[j].second),
                  Status::Ok);
      }
    };
...
TEST_F(EngineBasicTest, TestHashTableIterator) {
  ...
  auto MixedSet = [&](size_t id) {
    if (id % 2 == 0) {
      ...
      ASSERT_EQ(engine->SortedSet(collection_name, "sortedkey" + std::to_string(id),
                             "sortedval"),
                Status::Ok);
    }
  };
...
TEST_F(EngineBasicTest, TestExpireAPI) {
  ...
  // For sorte collection
  {
    ...
    ASSERT_EQ(s, Status::Ok);
    ASSERT_EQ(engine->SortedSet(sorted_collection, "sorted" + key, "sorted" + val),
              Status::Ok);
    // Set expired time for collection
    ASSERT_EQ(engine->Expire(sorted_collection, max_ttl_time),
              Status::InvalidArgument);
    ASSERT_EQ(engine->SortedSet(sorted_collection, "sorted2" + key, "sorted2" + val),
              Status::Ok);
  ...
  // For hashes collection
  {
    ...
    ASSERT_EQ(engine->Expire(hashes_collection, max_ttl_time),
              Status::InvalidArgument);
    ASSERT_EQ(
        engine->HashSet(hashes_collection, "hashes2" + key, "hashes2" + val),
        Status::Ok);
...
TEST_F(EngineBasicTest, TestSortedRecoverySyncPointCaseOne) {
  ...
  SyncPoint::GetInstance()->SetCallBack(
      "Test::SortedSet::Update::Finish", [&](void*) { update_num.fetch_add(1); });
  SyncPoint::GetInstance()->EnableProcessing();
  ...
  try {
    for (int i = 0; i < cnt; ++i) {
      if (i % 2 == 0) {
        engine->SortedSet(collection_name, "key" + std::to_string(i),
                     "val" + std::to_string(i));
      } else {
        std::string new_val = "val*" + std::to_string(i);
        engine->SortedSet(collection_name, "key" + std::to_string(i - 1), new_val);
        TEST_SYNC_POINT("Test::SortedSet::Update::Finish");
      }
    }
  } catch (...) {
    delete engine;
...
TEST_F(EngineBasicTest, TestSortedRecoverySyncPointCaseTwo) {
  ...
  try {
    engine->SortedSet(collection_name, keylists[0], "val" + keylists[0]);
    engine->SortedSet(collection_name, keylists[1], "val" + keylists[1]);
    engine->SortedSet(collection_name, keylists[2], "val" + keylists[2]);
    ...
    std::string got_val;
    ASSERT_EQ(engine->SortedGet(collection_name, keylists[0], &got_val), Status::Ok);
    ASSERT_EQ(engine->SortedGet(collection_name, keylists[2], &got_val), Status::Ok);
    ASSERT_EQ(got_val, "val" + keylists[2]);
    ASSERT_EQ(engine->SortedGet(collection_name, keylists[1], &got_val), Status::Ok);
...
TEST_F(EngineBasicTest, TestSortedSyncPoint) {
  ...
  engine->SortedSet(collection_name, "key0", "val0");
  engine->SortedSet(collection_name, "key2", "val2");
  ...
  ths.emplace_back(std::thread([&]() {
    engine->SortedSet(collection_name, "key1", "val1");
    std::string got_val;
