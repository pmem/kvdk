...
// Check if the edge is in edge or out edge, we need to distinguish the direction
// For detail steps:
//   1. Get the edge list
//   2. Check the vertex of the input
Status GraphSimulator::GetEdge(const Vertex &in, const Vertex &out, int direction, Edge &edge) {
	...
	if (direction == 1) {
		...
		for (auto &item : edge_list.edges) {
			if ((found = CheckEdgeExists(in, out, direction, item)))
				edge = item;
				break;
		}
...
Status GraphSimulator::AddEdge(Edge &edge) {
	...
	// edge's direction is important. If the direction < 2, for every edge's two vertex,
	// we need to store two vertexes : src --> dst  and dst --> src. Their key and value's
	// data is different in kuaishou's graph storage.
	Status s;
