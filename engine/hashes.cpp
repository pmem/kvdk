...
bool Skiplist::FindAndLockWritePos(Splice *splice, const Slice &insert_key, const HashTable::KeyHashHint &hint,
                                   std::vector<SpinMutex *> &spins, DLDataEntry *updated_data_entry)
{
    {
        spins.clear();
        DLDataEntry *prev;
        ...
        if (!updated_data_entry && (prev->next != next_offset || (next && next->prev != prev_offset)))
        {
            for (auto &m : spins)
            {
                m->unlock();
    ...
    if (node == nullptr)
    {
        ...
        for (int i = 1; i <= height; i++)
        {
            while (1)
            {
                ...
                if (insert_splice->prevs[i]->CASNext(i, insert_splice->nexts[i], node))
                {
                    break;
                }
