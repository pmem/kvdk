...
template <typename Lock> class UniqueLockTriplet {
private:
...
public:
  // Empty UniqueLockTriplet. Needs to swap with other UniqueLockTriplet to gain
  // actual Lock.
  UniqueLockTriplet() = default;

  UniqueLockTriplet(Lock &first, Lock &second, Lock &third, std::defer_lock_t)
      : _acquired_first_{false}, _acquired_second_{false},
        _acquired_third_{false}, _dup_second_{false}, _dup_third_{false} {
  ...
  UniqueLockTriplet(UniqueLockTriplet &&other)
      : _acquired_first_{other._acquired_first_},
        ...
        _dup_third_{other._dup_third_}, _first_{std::move(other._first_)},
        _second_{std::move(other._second_)}, _third_{std::move(other._third_)} {
  }
  ...
  bool TryLockAll() {
    if (_AcquiredNone_() && _OwnsNone_()) {
      ...
      throw std::runtime_error{"Invalid internal state of UniqueLockTriplet, "
                               "already owns lock but TryLockAll() called!"};
    }
  }
  ...
  void Unlock() {
    if (_AcquiredAll_() && _OwnsAll_()) {
      ...
      throw std::runtime_error{
          "Trying to Unlock locks not acquired yet in UniqueLockTriplet!"};
    } else {
      throw std::runtime_error{
          "Invalid internal state of UniqueLockTriplet, not all or none locks "
          "acquired when calling Unlock()."};
    }
  }
  ...
  bool OwnsLocks() {
    if (_AcquiredAll_() && _OwnsAll_()) {
      ...
      throw std::runtime_error{
          "Invalid internal state of UniqueLockTriplet, not all or none locks "
          "acquired when calling OwnsLocks()."};
    }
  }
