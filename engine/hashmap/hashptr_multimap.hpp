...
template <typename HashType, typename Pointer, size_t NSlot = 8,
          typename Alloc = std::allocator<Pointer>>
...
private:
  ...
  class Bucket;
  using BucketAllocatorType =
      typename std::allocator_traits<Alloc>::template rebind_alloc<Bucket>;
  using BucketAllocatorTraits =
      typename std::allocator_traits<Alloc>::template rebind_traits<Bucket>;
...
private:
  ...
  public:
    ...
    inline mask_type match_tag(tag_type tag) const noexcept {
      ...
      results.mask[0] =
          (_mm512_mask_cmpeq_epi8_mask(low_mask, low_tag, m512[0]) |
      ...
      if (n_logical_field == 16) // Compiler will remove branch
      {
        ...
        results.mask[1] =
            (_mm512_mask_cmpeq_epi8_mask(low_mask, low_tag, m512[1]) |
             _mm512_mask_cmpeq_epi8_mask(high_mask, high_tag, m512[1]));
      }

      // mask_type valid_fields = ~(empty[0] | (empty[1] << 8)) &
      // BucketMeta::bitmap_mask;
    ...
    inline Bucket *next_bucket() const noexcept { return meta.pointer(); }

    inline BucketMeta *mutex() noexcept { return &meta; }
    ...
    inline static void destroy_appended_buckets(Bucket *p_bucket,
    ...
    friend std::ostream &operator<<(std::ostream &out, Bucket const &bucket) {
      out << std::bitset<64>(bucket.meta.storage.load()) << "\n";
      for (size_t i = 0; i < 15; i++) {
        out << std::bitset<16>(bucket.data[i].tag()) << "\t"
...
public:
  ...
  class iterator {
    ...
    public:
      Pointer get() const noexcept { return p_bucket->get(idx); }

      void set(Pointer p) noexcept { p_bucket->reset(idx, p); }

      friend bool operator==(iterator_state lhs, iterator_state rhs) {
        return (lhs.p_bucket == rhs.p_bucket) && (lhs.tag == rhs.tag) &&
               (lhs.matches == rhs.matches) && (lhs.idx == rhs.idx);
      }
    };
  ...
  public:
    ...
    inline iterator_state &operator*() { return state; }

    inline iterator_state const &operator*() const { return state; }
  ...
  private:
    ...
    // Only two iterators at end or with exact internal state are considered
    // equal
    inline friend bool operator!=(iterator const &lhs, iterator const &rhs) {
      return !(lhs == rhs);
    }
...
public:
  ...
  explicit HashPointerMultimap()
      : n_bucket{0}, alloc{Alloc{}}, bucket_base{nullptr} {}

  explicit HashPointerMultimap(size_t n, Alloc const &a = Alloc{})
      : n_bucket{n}, alloc{a}, bucket_base{BucketAllocatorTraits::allocate(
                                   alloc, n_bucket)} {
    {
