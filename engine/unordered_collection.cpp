...
UnorderedCollection::UnorderedCollection(
    ...
    std::uint64_t id, std::uint64_t timestamp) try : _sp_hash_table_ {
  sp_hash_table
}
, _p_pmem_allocator_{sp_pmem_allocator.get()}, _pmp_dlist_record_{nullptr},
    _dlinked_list_{sp_pmem_allocator, timestamp, _ID2View_(id),
                   pmem::obj::string_view{""}},
    _name_{name}, _id_{id}, _time_stamp_{timestamp} {
  {
    auto space_list_record = _dlinked_list_._p_pmem_allocator_->Allocate(
        sizeof(DLDataEntry) + _name_.size() + sizeof(decltype(_id_)));
    ...
    void *pmp_list_record =
        _dlinked_list_._p_pmem_allocator_->offset2addr_checked(
            offset_list_record);
    DLDataEntry entry_list_record; // Set up entry with meta
    {
      ...
      entry_list_record.header.checksum =
          DLinkedList::_CheckSum_(entry_list_record, _name_, _ID2View_(_id_));

      entry_list_record.prev = _dlinked_list_.Head()._GetOffset_();
      entry_list_record.next = _dlinked_list_.Tail()._GetOffset_();
    }
    DLinkedList::_PersistRecord_(pmp_list_record, entry_list_record, _name_,
                                 _ID2View_(_id_));
...
UnorderedCollection::UnorderedCollection(
    ...
    : _sp_hash_table_{sp_hash_table},
      ...
      _name_{pmp_dlist_record->Key()}, _id_{_View2ID_(
                                           pmp_dlist_record->Value())},
...
EmplaceReturn UnorderedCollection::EmplaceBefore(
  ...
  EmplaceReturn ret =
      _EmplaceBetween_(iter_prev._pmp_curr_, iter_next._pmp_curr_, timestamp,
                       key, value, type, lock);
  return ret;
}
...
UnorderedIterator::UnorderedIterator(
    ...
    : _sp_coll_{sp_coll},
      _iterator_internal_{sp_coll->_dlinked_list_.Head()}, _valid_{false} {}
...
FATAL_FAILURE:
  throw std::runtime_error{
      "UnorderedCollection::DListIterator::_Prev_() fails!"};
}
} // namespace KVDK_NAMESPACE
