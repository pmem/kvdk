...
instance_space = 40 * 1024 * 1024 * 1024 
...
if __name__ == "__main__":
    ...
    num = data_size // (value_size + key_size + header_size)
    ...
    report = report_path + "hashes_vs{}_fill_thread{}".format(value_size, benchmark_threads)
    ...
    report = report_path + "hashes_vs{}_random_read_thread{}".format(value_size, benchmark_threads)
    print("Random read hashes-type kv")
    os.system("{0} {1} > {2}".format(exec, new_para, report))

    # zipf read
    new_para = para + " -fill=0 -read_ratio=1 -key_distribution=zipf"
    report = report_path + "hashes_vs{}_zipf_read_thread{}".format(value_size, benchmark_threads)
    print("Zipf read hashes-type kv")
    os.system("{0} {1} > {2}".format(exec, new_para, report))

    # insert new kvs
    new_para = para + " -fill=0 -read_ratio=0 -existing_keys_ratio=0"
    report = report_path + "hashes_vs{}_insert_thread{}".format(value_size, benchmark_threads)
    print("Insert new hashes-type kv")
    os.system("{0} {1} > {2}".format(exec, new_para, report))

    # random update
    new_para = para + " -fill=0 -read_ratio=0 -key_distribution=random"
    report = report_path + "hashes_vs{}_random_update_thread{}".format(value_size, benchmark_threads)
    print("Random update hashes-type kv")
    os.system("{0} {1} > {2}".format(exec, new_para, report))

    # zipf update
    new_para = para + " -fill=0 -read_ratio=0 -key_distribution=zipf"
    report = report_path + "hashes_vs{}_zipf_update_thread{}".format(value_size, benchmark_threads)
    print("Zipf update hashes-type kv")
    os.system("{0} {1} > {2}".format(exec, new_para, report))

    # range scan
    new_para = para + " -fill=0 -read_ratio=1 -scan=1"
    report = report_path + "hashes_vs{}_scan_thread{}".format(value_size, benchmark_threads)
    print("Scan hashes-type kv")
    os.system("{0} {1} > {2}".format(exec, new_para, report))

    # read + update
    new_para = para + " -fill=0 -read_ratio=0.9"
    report = report_path + "hashes_vs{}_ru91_thread{}".format(value_size, benchmark_threads)
