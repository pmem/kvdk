...
Status KVEngine::InitCollection(const StringView& collection, Collection** list,
  ...
  if (collection_type != RecordType::SortedHeaderRecord) {
  ...
  auto space = pmem_allocator_->GuardedAllocate(request_size);
  if (space.Size() == 0) {
    return Status::PmemOverflow;
  }
  CollectionIDType id = list_id_.fetch_add(1);
  DLRecord* pmem_record = DLRecord::PersistDLRecord(
      space, version_controller_.GetCurrentTimestamp(),
      (RecordType)collection_type, kNullPMemOffset, space.Offset(),
      space.Offset(), collection, StringView((char*)&id, 8));

  std::lock_guard<std::mutex> guard{list_mu_};
  skiplists_.push_back(
      std::make_shared<Skiplist>(pmem_record, string_view_2_string(collection),
...
Status KVEngine::SDeleteImpl(Skiplist* skiplist, const StringView& user_key) {
  ...
  DLRecord* delete_record = nullptr;
  SkiplistNode* dram_node = nullptr;
  ...
  while (1) {
    ...
    switch (s) {
      ...
      case Status::Ok: {
        if (hash_entry.header.data_type == SortedDeleteRecord) {
          ...
          if (space.Size() == 0) {
            space = pmem_allocator_->GuardedAllocate(sizeof(DLRecord) +
                                                     collection_key.size());
            if (space.Size() == 0) {
              return Status::PmemOverflow;
            }
            delete_record = static_cast<DLRecord*>(space.Address());
          }
          break;
        }
      }
    ...
    if (dram_node == nullptr) {
      hash_table_->Insert(hint, entry_ptr, SortedDeleteRecord, delete_record,
    ...
    delayFree(OldDeleteRecord{delete_record, new_ts, entry_ptr, hint.spin});
    break;
  }
  return Status::Ok;
}
...
Status KVEngine::SSetImpl(Skiplist* skiplist, const StringView& user_key,
  ...
  while (1) {
    ...
    HashEntry* entry_ptr = nullptr;
    DLRecord* existing_record = nullptr;
    DLRecord* new_record = static_cast<DLRecord*>(space.Address());
    HashEntry hash_entry;
    DataEntry data_entry;
    auto hint = hash_table_->GetHint(collection_key);
    std::unique_lock<SpinMutex> ul(*hint.spin);
    ...
    if (found) {
      ...
      if (dram_node == nullptr) {
        hash_table_->Insert(hint, entry_ptr, SortedDataRecord, new_record,
                            HashIndexType::DLRecord);
      } else {
      ...
      hash_table_->Insert(
          hint, entry_ptr, SortedDataRecord,
          (dram_node == nullptr) ? static_cast<void*>(new_record) : dram_node,
...
Status KVEngine::BatchWrite(const WriteBatch& write_batch) {
  ...
  std::vector<PMemOffsetType> space_entry_offsets(write_batch.Size());

  for (size_t i = 0; i < write_batch.Size(); i++) {
    auto& kv = write_batch.kvs[i];
    std::uint32_t alloc_size;
    if (kv.type == StringDataRecord) {
      alloc_size = kv.key.size() + kv.value.size() + sizeof(StringRecord);
    } else if (kv.type == StringDeleteRecord) {
      alloc_size = kv.key.size() + sizeof(StringRecord);
    } else {
      GlobalLogger.Error("only support string type batch write\n");
      return Status::NotSupported;
    }
    batch_hints[i].allocated_space =
        pmem_allocator_->GuardedAllocate(alloc_size);
    if (batch_hints[i].allocated_space.Size() == 0) {
      return Status::PmemOverflow;
    }
    space_entry_offsets[i] = batch_hints[i].allocated_space.Offset();
...
Status KVEngine::StringBatchWriteImpl(const WriteBatch::KV& kv,
...

  {
    ...
    void* block_base = batch_hint.allocated_space.Address();

    TEST_SYNC_POINT(
        "KVEngine::BatchWrite::StringBatchWriteImpl::Pesistent::Before");

    StringRecord::PersistStringRecord(
        batch_hint.allocated_space, batch_hint.timestamp,
...

  {
    ...
    switch (s) {
      case Status::Ok: {
        ...
        StringRecord* new_record = StringRecord::PersistStringRecord(
            space, new_ts, StringDeleteRecord,
            pmem_allocator_->addr2offset_checked(
                hash_entry.index.string_record),
            key, "");

        hash_table_->Insert(hint, entry_ptr, StringDeleteRecord, new_record,
                            HashIndexType::StringRecord);
        ul.unlock();
        ...
        delayFree(OldDeleteRecord{new_record, new_ts, entry_ptr, hint.spin});
...
Status KVEngine::StringSetImpl(const StringView& key, const StringView& value) {
  ...
  auto space = pmem_allocator_->GuardedAllocate(requested_size);
  if (space.Size() == 0) {
    return Status::PmemOverflow;
  }

  {
    ...
    // Write hash index
    hash_table_->Insert(hint, hash_entry_ptr, StringDataRecord, new_record,
                        HashIndexType::StringRecord);
