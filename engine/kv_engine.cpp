...
Status KVEngine::RestoreData() {
  ...
  while (true) {
    ...
    if (data_entry_cached.header.record_size == 0) {
      ...
      recovering_pmem_data_entry->meta.mark.record_type = RecordType::Empty;
    ...
    switch (data_entry_cached.meta.mark.record_type) {
      ...
      case RecordType::ListElem: {
        if (data_entry_cached.meta.mark.record_status == RecordStatus::Dirty) {
          data_entry_cached.meta.mark.record_type = RecordType::Empty;
        } else {
    ...
    if (data_entry_cached.meta.mark.record_type == RecordType::Empty) {
    ...
    while (slot_iter.Valid()) {
      ...
      switch (mark.record_type) {
        case RecordType::String: {
          ...
          if (record &&
              record->GetRecordMark().record_status == RecordStatus::Normal &&
              !record->HasExpired()) {
        ...
        case RecordType::SortedHeader: {
          ...
          if (header &&
              header->GetRecordMark().record_status == RecordStatus::Normal &&
              !header->HasExpired()) {
...
HashTable::LookupResult KVEngine::lookupKey(StringView key, uint8_t type_mask) {
  ...
  } else if (mark.record_status == RecordStatus::Outdated) {
